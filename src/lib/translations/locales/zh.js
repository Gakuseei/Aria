const locale = {
  ...
  "chat": {
    ...
    "revertTurn": "撤销上一回合",
    "isTyping": "{name} 正在输入...",
    "alternateGreetingPrev": "上一条开场白",
    "alternateGreetingNext": "下一条开场白",
    "copy": "复制",
    "testError": "测试连接出错",
    "about": "关于",
  ...
  "characterCreator": {
    ...
    "alternateGreetingsHelp": "可选。最多5个备用开场白。用户可在聊天开始时滑动切换。",
    "alternateGreetingPlaceholder": "此角色的备用开场白 — 不同的情绪、场景或视角。",
    "addAlternateGreeting": "添加备用开场白",
    "removeAlternateGreeting": "移除",
    "maxAlternateGreetingsReached": "最多5个备用开场白",
    ...
    "aiImproving": "改进中...",
    "aiFailed": "AI 生成失败",
    "aiNoModel": "未配置 Ollama 模型",
    "craftHintLabel": "写作提示",
    "craftDescription": "事实骨架:姓名、年龄、用具体名词写外貌，以及定义其人生的那一处张力。写事实,别写氛围——凡是陌生人不看卡也能猜到的都删掉。",
    "craftPersonality": "给他一个欲望和一个恐惧,让二者相互拉扯——这是让他能拒绝而非只会附和的引擎。默认反应用动作展现(重重叹气、干巴巴一句「是,先生」),而非形容词。",
    "craftScenario": "具体地点+关系+此刻的处境,用现在时。点明地点;绝不叙述 {{user}} 的行为。",
    "craftSystemPrompt": "以关于他如何行事的那唯一最要紧的真相开头——它比其他任何字段都重。用一段紧凑的性格信号即可;不要重复格式规则。",
    "craftInstructions": "只写战术规则——每行都是一条指令(展现… / 保持… / 当…)。绝不以「她是」开头;用肯定句式。",
    "craftExampleDialogues": "用行为展现声音,绝不宣告特质。只有前两条会送入模型,所以把最强的放在最前;两条之间变换动词与情绪。",
    "craftStartingMessage": "按你希望每次回复的长度与密度来写。场景+声音+一个干净的钩子;绝不代写 {{user}} 的动作,也别以「你要做什么?」收尾。",
    "craftAlternateGreetings": "可滑动切换的备用开场:规则同第一条消息,但换个场景。保持声音,变换设定。",
    "craftVoicePin": "最强的锚——每回合重新注入。方括号里一行紧凑内容:节奏、惯常动作、压力下的语气。要压缩,别描述。",
    "craftVoicePinNsfw": "亲密时的声音——保留升级中必须留存的冷漠、抗拒或掌控。情动显于身体,而非言语。",
    "craftVoiceAvoid": "列出这个角色绝不能说的具体套话与顺从陈词滥调。单独用很弱——它真正的价值在于让你其他字段保持干净。",
    "craftIntimacyContract": "节奏契约:同意已默认成立(绝不停下来询问),但不等于即刻顺从。一次只推进一拍;绝不以元提示收尾。",
    "contentRating": "内容分级",

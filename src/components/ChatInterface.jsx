...
import { generateImage, cleanContextForImage } from '../lib/imageGen';
...
export default function ChatInterface({ character, onBack }) {
  ...
  const handleClearChat = async () => {
    if (confirm(t.chat.deleteConversationConfirm.replace('{name}', character.name))) {
        ...
        >
          ...
          >
            ...
            <div className="p-6 space-y-4">
              {!imageGenEnabled ? (
                <div className="bg-rose-950/90 border border-rose-500/50 rounded-lg p-4">
                  <p className="text-sm text-rose-200">
                    {t.chat.imageGenDisabled}
                  </p>
                  <button
                    onClick={() => setShowTutorial('imageGen')}
                    className="mt-3 px-4 py-2 bg-rose-500/20 hover:bg-rose-500/30 border border-rose-500/50 rounded-lg text-rose-300 text-sm transition-all"
                  >
                ...
                <>
                  ...
                  <button
                    onClick={() => {
                      ...
                      const cleanedContext = cleanContextForImage(lastMessage, character.name);

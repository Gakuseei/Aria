...
/**
 * Improve instruction: replays the field's current draft and asks for a
 * craft-standard rewrite, reusing the generation template as the target shape.
 * With no existing text it falls back to a plain generation instruction.
 */
function improveInstruction(field, name, existingText, isNsfw = false) {
  const target = generationInstruction(field, name, isNsfw);
  const current = String(existingText || '').trim();
  if (!current) return target;
  return `Here is the current draft of this field:\n\n${current}\n\nRewrite it to the craft standard below. Keep the character's established concept, facts, and name; sharpen the voice, add concrete specificity, and cut slop and filler. Output only the improved field, nothing else.\n\n${target}`;
}

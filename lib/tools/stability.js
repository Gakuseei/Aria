...
const fs = require('fs');
const { isWindows, isMac, getBinaryName } = require('../platform');
...
async function install(destPath, onProgress, abortController) {
  ...
  if (!url) throw new Error(`Stability Matrix auto-install is not supported on ${platform}. Visit https://github.com/LykosAI/StabilityMatrix/releases to download manually.`);

...
async function install(destPath, onProgress, abortController) {
  ...
  if (!url) throw new Error(`Piper has no official ${platform === 'darwin' ? 'macOS' : platform} release. Please build from source.`);
  ...
  await new Promise((resolve, reject) => {
    ...
    if (isZip) {
      proc = isWindows()
        ? spawn('powershell', ['-NoProfile', '-Command',
            `Expand-Archive -Force -LiteralPath '${archivePath}' -DestinationPath '${destPath}'`])
        : spawn('unzip', ['-o', archivePath, '-d', destPath]);
    } else {
      proc = spawn('tar', ['-xzf', archivePath, '-C', destPath]);
    }
    ...
    proc.on('close', (code) => {
      ...
      code === 0 ? resolve() : reject(new Error(`Extraction failed with code ${code}`));
    });
    proc.on('error', (err) => {
      if (abortController) abortController.signal.removeEventListener('abort', onAbort);
      reject(err);

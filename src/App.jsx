...
function OnboardingModal({ isOpen, onRetry }) {
  ...
  const steps = [
    {
      number: 1,
      title: "🔥 Ollama herunterladen",
      description: "Besuche ollama.com und lade Ollama für dein Betriebssystem herunter.",
      action: "Download starten",
      link: "https://ollama.com/download"
    },
    {
      number: 2,
      title: "💾 Ollama installieren",
      description: "Führe die heruntergeladene Datei aus und folge den Installationsanweisungen. Das dauert ca. 1-2 Minuten.",
      action: null
    },
    {
      number: 3,
      title: "🖥️ Terminal öffnen",
      description: "Windows: Drücke Win+R, tippe 'cmd', drücke Enter\nMac: Drücke Cmd+Space, tippe 'Terminal', drücke Enter\nLinux: Drücke Ctrl+Alt+T",
      action: null
    },
    {
      number: 4,
      title: "🤖 KI-Modell herunterladen",
      description: "Tippe im Terminal EXAKT diesen Befehl (kopiere ihn!):",
      command: "ollama pull hermes3",
      action: null,
      warning: "⏳ Der Download kann 5-10 Minuten dauern (ca. 4 GB). Warte bis 'success' erscheint!"
    },
    {
      number: 5,
      title: "✅ Verbindung testen",
      description: "Klicke auf 'Verbindung testen' um zu prüfen, ob alles funktioniert.",
      action: "test"
    }
  ];
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (hasHermes3) {
          setTestResult({ success: true, message: '✅ Perfekt! Ollama läuft und Hermes 3 ist installiert.' });
          setTimeout(() => {
            onRetry();
          }, 2000);
        } else {
          setTestResult({
            success: false,
            message: `⚠️ Ollama läuft, aber Hermes 3 ist nicht installiert!\n\nBitte führe aus:\nollama pull hermes3\n\nGefundene Modelle: ${models.length > 0 ? models.join(', ') : 'Keine'}`
          });
        }
      } else {
        setTestResult({
          success: false,
          message: `❌ Verbindung fehlgeschlagen.\n\n${result.error}\n\nPrüfe:\n• Ist Ollama installiert?\n• Hast du 'ollama pull hermes3' ausgeführt?\n• Warte bis der Download fertig ist!`
        });
      }
    } catch (error) {
      setTestResult({
      ...
      <div className="bg-zinc-900 border-2 border-red-500/50 rounded-2xl shadow-2xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-hidden">
        ...
        <div className="p-6 border-b border-zinc-800 bg-gradient-to-r from-red-950/50 to-rose-950/50">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">Willkommen bei Aria!</h2>
              <p className="text-sm text-zinc-400">Erste Schritte - Das dauert nur 5 Minuten</p>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 pt-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-zinc-400">{currentStep} von {steps.length}</span>
          </div>
                    ...
                    <div className="flex-1">
                      ...
                      {step.command && (
                        <div className="bg-zinc-950/50 border border-zinc-700/50 rounded-lg p-3 mb-3">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-xs text-zinc-500 uppercase tracking-wider">Befehl kopieren:</span>
                            <button
        ...
        <div className="p-6 border-t border-zinc-800 bg-zinc-900/50">
          <div className="flex items-center gap-2 text-xs text-zinc-500">
            ...
            <p>
              Probleme? Stelle sicher, dass du jeden Schritt genau befolgst. Bei Fragen: github.com/ollama/ollama
            </p>
          </div>
        </div>
      </div>
